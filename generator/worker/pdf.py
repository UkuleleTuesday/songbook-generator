...
def scan_drive_editions(
    ...
    with tracer.start_as_current_span("scan_drive_editions") as span:
        ...
        logger.info(
            f"scan_drive_editions: starting Drive scan for .songbook.yaml files; "
            f"source_folders={source_folders!r}"
        )
        ...
        logger.info(
            f"scan_drive_editions: Drive query returned {len(songbook_files)} "
            f".songbook.yaml file(s)"
        )
        ...
        for f in songbook_files:
            logger.debug(
                f"scan_drive_editions: processing file id={f.id!r} "
                f"name={f.name!r} parents={f.parents!r}"
            )
            ...
            if not folder_id:
                skipped_no_parent += 1
                logger.warning(
                    f"scan_drive_editions: .songbook.yaml id={f.id!r} has no "
                    f"parent folder, skipping"
                )
                click.echo(
            ...
            try:
                logger.debug(
                    ...
                    f"folder_id={folder_id!r} file_id={f.id!r}"
                )
                raw = gdrive_client.download_raw_bytes(f.id)
                data = yaml.safe_load(raw.decode("utf-8"))
                logger.debug(
                    f"scan_drive_editions: parsed YAML from folder_id={folder_id!r}: "
                ...
                logger.warning(
                    ...
                    f"folder_id={folder_id!r} file_id={f.id!r}: {e}"
                )
                click.echo(
                ...
                skipped_errors += 1
                logger.warning(
                    f"scan_drive_editions: .songbook.yaml in folder_id={folder_id!r} "
                    f"failed Edition schema validation: {e}"
                )
                click.echo(
        ...
        logger.info(
            ...
            f"skipped_no_parent={skipped_no_parent} skipped_errors={skipped_errors}"
        )
        return editions

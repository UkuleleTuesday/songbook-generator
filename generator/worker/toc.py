...
class TocGenerator:
    ...
    def _draw_marks(page: fitz.Page, marks: list) -> None:
        ...
        Each mark is ``(x, baseline_y, width, height, stripes, weights,
        vertical)`` — the argument list of :func:`badges.draw_flag`, deferred so
        the marks are painted once the page is finalised.
        """
        for mark in marks:
            badges.draw_flag(page, *mark)
    ...
    def _add_toc_entry(
        ...
        entry_badges, entry_color = badges.collect_decoration_badges(
        ...
        for badge in entry_badges:
            if badge.text is not None:
                ...
                stripes, weights = badges.FLAG_PALETTES[badge.symbol]
                vertical = badge.symbol in badges.VERTICAL_FLAG_SYMBOLS
                flag_w, flag_h, gap = self._flag_size()
                x += gap
                marks.append((x, y_pos, flag_w, flag_h, stripes, weights, vertical))
                x += flag_w + gap

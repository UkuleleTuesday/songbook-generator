...
def mock_google_services():
    ...
    with patch('cover.default') as mock_default, \
...
def test_create_cover_from_template_basic(mock_google_services):
    ...
    docs.documents.return_value.batchUpdate.return_value.execute.return_value = batch_response
...
def test_create_cover_from_template_missing_occurrences_changed(mock_google_services, capsys):
    ...
    result = cover.create_cover_from_template('template123', {'{{DATE}}': '1st January 2024'})
...
def test_generate_cover_basic(mock_now, mock_load_cover_config, mock_google_services):
    ...
    with patch('cover.os.makedirs'), \
        ...
        mock_create_cover.assert_called_once_with(cover_file_id, {'{{DATE}}': '1st January 2024'})
        ...
        mock_drive.files.return_value.delete.assert_called_once_with(fileId='temp_cover123')
...
def test_generate_cover_no_cover_configured(mock_echo, mock_load_cover_config):
    ...
    mock_echo.assert_called_once_with("No cover file ID configured. Skipping cover generation.")
...
def test_generate_cover_deletion_failure(mock_now, mock_load_cover_config, capsys):
    ...
    with patch('cover.os.makedirs'), \
        ...
        mock_drive.files.return_value.delete.return_value.execute.side_effect = Exception("API Error")
...
def test_generate_cover_uses_provided_cover_id(mock_now):
    ...
    with patch('cover.config.load_cover_config') as mock_load_cover_config, \
         patch('cover.os.makedirs'), \
         patch('cover.open', mock_open()), \
         patch('cover.fitz.open') as mock_fitz_open, \
         patch('cover.create_cover_from_template') as mock_create_cover:

        mock_load_cover_config.return_value = "config_cover123"  # This should be ignored
        mock_create_cover.return_value = 'temp_cover123'
        mock_pdf = Mock()
        mock_fitz_open.return_value = mock_pdf
        mock_drive.files.return_value.delete.return_value.execute.return_value = {}

        result = cover.generate_cover(mock_drive, mock_cache_dir, provided_cover_id)

        # Should use provided ID, not config ID
        mock_create_cover.assert_called_once_with(provided_cover_id, {'{{DATE}}': '1st January 2024'})

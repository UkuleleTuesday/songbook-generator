...
def test_gdrive_song_sheets_folder_ids_override(monkeypatch):
    """Test that SONG_SHEETS_FOLDER_IDS overrides song_sheets.folder_ids."""
    monkeypatch.setenv("SONG_SHEETS_FOLDER_IDS", "folder1,folder2")
    config.get_settings.cache_clear()
    settings = config.get_settings()
...
def test_gcs_worker_cache_bucket_override(monkeypatch):
    """Test that CACHING_GCS_WORKER_CACHE_BUCKET overrides caching.gcs.worker_cache_bucket."""
    monkeypatch.setenv("CACHING_GCS_WORKER_CACHE_BUCKET", "my-test-bucket")
    config.get_settings.cache_clear()
    settings = config.get_settings()
...
def test_gcp_region_override(monkeypatch):
    """Test that CACHING_GCS_REGION overrides caching.gcs.region."""
    monkeypatch.setenv("CACHING_GCS_REGION", "my-test-region")
    config.get_settings.cache_clear()
    settings = config.get_settings()
...
def test_local_cache_enabled_override(monkeypatch, env_value, expected_bool):
    """Test that CACHING_LOCAL_ENABLED overrides caching.local.enabled."""
    monkeypatch.setenv("CACHING_LOCAL_ENABLED", env_value)
    config.get_settings.cache_clear()
    settings = config.get_settings()
...
def test_local_cache_dir_override(monkeypatch):
    """Test that CACHING_LOCAL_DIR overrides caching.local.dir."""
    monkeypatch.setenv("CACHING_LOCAL_DIR", "/tmp/my-cache")
    config.get_settings.cache_clear()
    settings = config.get_settings()
